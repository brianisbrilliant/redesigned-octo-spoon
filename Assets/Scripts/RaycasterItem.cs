...
public class RaycasterItem : MonoBehaviour, IItem
{
    ...
    public void SecondaryAction() {
        // spherecasting. it's just a raycast, but it has a radius!
    ...
    IEnumerator Select() {
        while(true) {
            ...
            if(selecting) {
                ...
                lr.SetPosition(1, emitter.position + (emitter.forward * 100));

                RaycastHit hit;
                if(Physics.SphereCast(emitter.position, 1, emitter.forward, out hit, 100)) {
                    if(hit.transform.gameObject.CompareTag("Cube")) {
                        if(lastTouchedCube == hit.transform.gameObject) {
                            ...
                            // change 'lastTouchedCube' to the cube we're looking at now
